...
_centerMapOnMarker (markerIndex) {
  ...
  async fetchDispose() {
    let JsonData = await fetch('https://lisheapp.co.ke/backend/dispose.php');
  ...
  async fetchRecycle() {
    let JsonData = await fetch('https://lisheapp.co.ke/backend/recycle.php');
    let recycle = await JsonData.json();
    this.setState({recycle});
    const points = [];
    let count = 0;
    recycle.forEach(function(element, i) {
      let latitude = parseFloat(element.lat);
      let longitude = parseFloat(element.lng);
      let ide = element.id;
      let products = element.keyOff;
      points.push({ id: ide, title: element.company, subtitle:element.description, coordinate: {latitude,longitude}, type: element.type})
    });
    this.setState({ pins: this.state.pins.concat(points) })
  }

  async fetchReuse() {
    let JsonData = await fetch('https://lisheapp.co.ke/backend/reuse.php');
    let reuse = await JsonData.json();
    this.setState({reuse});
    const points = [];
    let count = 0;
    reuse.forEach(function(element, i) {
      let latitude = parseFloat(element.lat);
      let longitude = parseFloat(element.lng);
      let ide = element.id;
      let products = element.keyOff;
  ...
  mainExample (number, title) {
    ...
    return (
        <View style={[styles.exampleContainer, true ? styles.exampleContainerDark : styles.styles.exampleContainerDark]}>
            ...
            <GestureRecognizer
                onSwipeDown={(item) => this.onSwipeDown(item)}
                config={config}
            >
            <Carousel
