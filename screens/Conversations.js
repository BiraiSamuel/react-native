...
import { View, FlatList, ActivityIndicator, TouchableOpacity,Dimensions, AsyncStorage, TextInput } from 'react-native';
...
class Conversations extends Component {
  ...
  async makeRemoteRequest (id) {
    ...
    return await fetch('https://lisheapp.co.ke/get_contacts.php', {
        method: 'POST',
        body: JSON.stringify({
    ...
    return (
      <View style={{ flex: 1 }} backgroundColor='white'>
        <FlatList
          ...
          renderItem={({ item }) => (
            <ListItem
              leftAvatar={{ source: { uri: url + item.profile } }}
              title={`${item.user}`}
              subtitle={item.date}
              onPress={() => navigation.navigate('Conversation', {convo: item})}
            >
            </ListItem>
          )}
